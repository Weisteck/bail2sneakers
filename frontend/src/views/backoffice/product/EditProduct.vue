<template>
  <div class="container mx-auto">
    <form class="card" @submit.prevent="editProduct" ref="editProduct">
      ...
      <div class="text-center">
        <button type="submit" class="mt-10 btn-primary">EDIT PRODUCT</button>
      </div>
    </form>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    async getProduct() {
      try {
        const product = await this.$store.dispatch('getProduct', { id: this.$route.params.id })
    ...
    async editProduct() {
      try {
        const editProduct = await this.$store.dispatch('editProduct', {
